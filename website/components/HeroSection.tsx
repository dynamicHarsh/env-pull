...
export type WorkflowMode = "local" | "onepassword";
...
export default function HeroSection({ workflowMode, setWorkflowMode }: HeroSectionProps) {
  ...
  return (
    <section className="w-full flex flex-col items-center justify-center pt-32 pb-20 px-4 text-center">
      ...
      <p className="text-lg md:text-xl text-zinc-400 max-w-2xl mx-auto mb-10 leading-relaxed">
        Ditch <span className="font-mono text-zinc-300">.env</span>. Keep your flow.
      </p>
      ...
      <div className="flex items-center justify-center space-x-1 bg-zinc-900/50 p-1 rounded-full mb-3 w-max mx-auto border border-zinc-800">
        {(["mac", "linux"] as const).map((platform) => (
          <button
            key={platform}
      ...
      <div className="flex items-center justify-center space-x-1 bg-zinc-900/50 p-1 rounded-full mb-3 w-max mx-auto border border-zinc-800">
        {([
          ...
          >
            {workflowMode === key && (
              <motion.div
                layoutId="workflow-slider"
                className="absolute inset-0 bg-zinc-700/50 rounded-full -z-10"
                transition={{ type: "spring", stiffness: 300, damping: 30 }}
              />
            )}
            <span className={workflowMode === key ? "text-zinc-100" : "text-zinc-500 hover:text-zinc-300"}>
              {label}
            </span>
          </button>
        ))}
      ...
      {/* Terminal demo */}
      <TerminalAnimation workflowMode={workflowMode} />
    </section>
  );
}
